...
const Airtime = () => {
    ...
    const handlePinConfirm = async () => {
        ...
        try {
            ...
            // 2. Proceed with airtime purchase
            const res = await axios.post('https://server-mi7c.onrender.com/airtime', {
                ...form,
            ...
            <div className="w-full max-w-md bg-white/90 rounded-3xl shadow-2xl border border-white/50 p-8 backdrop-blur-xl relative overflow-hidden">
                <div className="flex items-center justify-center mb-6">
                    <span className="inline-flex items-center justify-center w-14 h-14 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl shadow-lg">
                        <span className="text-white font-bold text-2xl">{<Phone/>}</span>
                    </span>
                </div>
                <h2 className="text-2xl font-bold text-gray-900 mb-2 text-center">Buy Airtime</h2>
                <p className="text-center text-gray-500 mb-6">Top up any phone instantly</p>
                <form className="space-y-5" onSubmit={handleSubmit} autoComplete="off">
                    ...
                    {success && (
                        <div className="flex items-center justify-center text-green-600 font-semibold mt-2 gap-2 animate-fadeIn">
                            <span className="w-5 h-5 inline-block">✔️</span> Airtime purchase successful!
                        </div>
                    )}
                    {error && (
